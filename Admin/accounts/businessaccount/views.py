        ...
        if acc_type == 'for_new_acc':
            ...
            for accnumber in accnumber_list:
                if Accounts.objects.filter(isdeleted=False,accountno=accnumber).exists():
                    context['message'] = f'Account number {accnumber} already exist'
                    return render(request,'accounts/openaccount/business/business_account_register.html',context)
            try:
                sum([float(balance) for balance in balance_list])
            except Exception as e:
                logger.info(e)
                context['message'] = 'Balance should be in numbers'
                return render(request,'accounts/openaccount/business/business_account_register.html',context)
            if '1' not in main_owner_list_new or main_owner_list_new.count('1') > 1 or len(email_list) != len(set(email_list)) or request.session.get('email-err'):
                user_list_main=[]
                for i in range(len(firstName_list)):
                ...
                context['message'] = 'Account numbers should not be empty'
                return render(request,'accounts/openaccount/business/business_account_register.html',context)
            elif not all(balance_list):
                context['message'] = 'Balance numbers should not be empty'
                return render(request,'accounts/openaccount/business/business_account_register.html',context)
            elif not all(currency_list):
                context['message'] = 'Currency should be selected'
                return render(request,'accounts/openaccount/business/business_account_register.html',context)
            elif len(accnumber_list) != len(accnumber_set):
                context['message'] = 'Account numbers should not be same'
                return render(request,'accounts/openaccount/business/business_account_register.html',context)
            elif len(currency_list) != len(currency_set):
                context['message'] = 'Please choose a different currency'
                return render(request,'accounts/openaccount/business/business_account_register.html',context)
            elif not all([accnumber.isdigit() for accnumber in accnumber_list]):
                context['message'] = 'Account number should be a whole number'
                return render(request,'accounts/openaccount/business/business_account_register.html',context)

            elif len(tuple(accnumber_and_bal)) < 2:
                context['message'] = 'Minimum 2 accounts details should be filled'
                return render(request,'accounts/openaccount/business/business_account_register.html',context)
            else:
                user_list_main=[]
                for i in range(len(firstName_list)):
...
class BusinessAccountConfirmView(View,ConfirmYourMail):
    def get(self,request):
        if request.session.get('businessaccount'):
            # if request.session['businessaccount']['account_lists']:
            if request.session['businessaccount'].get('account_lists'):
                return render (request,'accounts/openaccount/business/business_account_register_confirm.html')
            else:
                context={
    ...
    def post(self,request):
        if request.session.get('businessaccount'):
            ...
            if request.session['businessaccount'].get('account_lists'):
                count = 1
                for acc in request.session['businessaccount']['account_lists']:
    ...
    def post(self,request):
        if request.POST.get('action')=='add':
            ...
            if User.objects.filter(email=user_email).exists():
                ...
                customer_doc_current = Customerdocuments.objects.get(customer__user=request.user,verificationtype__verificationtype=3)
                customer_doc_details_current1 = Customerdocumentdetails.objects.get(customerdocument=customer_doc_current,
                            field = Documentfields.objects.get(fieldtype='2',documenttype__description="Company registration document"))
                customer_doc_details_current2 = Customerdocumentdetails.objects.get(customerdocument=customer_doc_current,
                                field = Documentfields.objects.get(fieldtype='5',documenttype__description="Company registration document"))
                customer_doc_file_current = Customerdocumentfiles.objects.get(customerdocument=customer_doc_current)
