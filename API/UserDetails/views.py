...
from utils.models import Useraccounts, Countries, Otps
import logging
...
class PersonalDetailsView(View,OTP):
    ...
    def post(self, request):
        ...
        self.send_email_otp(transaction_type=7, created_by=request.user, email=request.user.email, full_name=full_name, token=token, activation_code=None)
    ...
    def post(self, request):
        ...
        if request.POST.get("action") == "resend":
            try:
                ...
                full_name = f"{user_account.firstname} {user_account.lastname}"
                self.send_email_otp(transactiontype_index, created_by=user, email=user.email, full_name=full_name, token=token,
...
class ChangePassword(View):
    ...
    def post(self,request):
        ...
        if request.POST.get('action_type') == 'otp_verfy':
            ...
            otp_status=OTP().send_email_otp(transaction_type=6,created_by=request.user,email=request.user.email,
                                        full_name=full_name,token=request.session.get('changepassword'))
            if otp_status:
                if request.session.get('prevPasswordToken'):
                    del request.session['prevPasswordToken']
...
class VerificationOTPSendView(View,OTP):
    def post(self,request):
        ...
        user_account = Useraccounts.objects.get(customer__user=request.user)
        full_name = f"{user_account.firstname} {user_account.lastname}"
        self.send_email_otp(transaction_type=1, created_by=request.user, email=request.user.email, full_name=full_name,
                            token=token, activation_code=None)
        return redirect('/verifyOTP')
...
class ReferFriend(View,CustomMail):
    ...
    def post(self, request):
        ...
        if data.get('status'):
            ...
            refer_link = f"{settings.CURRENT_DOMAIN}/refer/{user_account.referencecode}"
            content = f"{refer_link}?firstName={firstName}&lastName={lastName}&email={email}"
            self.send_refferal_mail(subject,content,email)
            context['link'] = refer_link
            context['status'] = True
