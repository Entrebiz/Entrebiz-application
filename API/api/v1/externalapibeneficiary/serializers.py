...
from api.v1.beneficiary.serializers import InternalbeneficySerializer
...
class ExternalApibeneficiarySerializer(serializers.ModelSerializer):
    accountnumber = serializers.CharField(required=False)
    name = serializers.CharField(required=False)
    swiftcode = serializers.CharField(required=False)
    city = serializers.CharField(required=False)
    bankname = serializers.CharField(required=False)
    currency_code = serializers.CharField(source="currency.code", required=False)
    country_code = serializers.CharField(source="country.shortform", required=False)
    slug = ReadOnlyField()
    ...
    def create(self, validated_data):
        ...
        externalben_obj, created = Externalbeneficiaries.objects.get_or_create(name=name, accountnumber=accountnumber,
                                                                               currency=Currencies.objects.get(
                                                                                   code=currency_code),
                                                                               swiftcode=swiftcode,
                                                                               country=Countries.objects.get(
                                                                                   shortform=country_code), city=city,
                                                                               bankname=bankname,
                                                                               createdby=user, customer=customer,
                                                                               email=email, isdeleted=False)
        return externalben_obj

    def update(self, instance, validated_data):
        instance.currency = Currencies.active.get(code=validated_data["currency"].get('code', instance.currency.code) if validated_data.get("currency") else instance.currency.code)
        instance.country = Countries.objects.get(shortform=validated_data["country"].get('shortform', instance.country.shortform) if validated_data.get("country") else instance.country.shortform)
    ...
    class Meta:
        ...
        fields = ("slug", "accountnumber", "name", "currency_code", "swiftcode", "city", "bankname", "country_code", "email")


class InternalApibeneficySerializer(serializers.ModelSerializer):
    accountnumber = serializers.CharField(source="account.accountno", required=False)
    first_name = serializers.CharField(source="account.user_account.firstname", required=False)
    last_name = serializers.CharField(source="account.user_account.lastname", required=False)
    currency_code = ReadOnlyField(source="account.currency.code", required=False)
    receivername = serializers.CharField(required=False)
    slug = ReadOnlyField()

    def __init__(self, *args, **kwargs):
        super(InternalApibeneficySerializer, self).__init__(*args, **kwargs)
    ...
    def to_representation(self, instance):
        ...
        if instance.account.user_account.customer.customertype == 1:
            ...
            try:
                representation['first_name'] = Businessdetails.objects.get(
                    customer=instance.account.user_account.customer).companyname
            except:
    ...
    def create(self, validated_data):
        ...
        internalben_obj, created = Internalbeneficiaries.objects.get_or_create(receivername=receivername,
                                                                               account=account_obj, createdby=user,
                                                                               modifiedby=user,
    ...
    def update(self, instance, validated_data):
        if validated_data.get("account"):
            instance.account = Accounts.active.get(accountno=validated_data.get("account").get('accountno', instance.account.accountno))
        if validated_data.get("receivername"):
    ...
    def validate(self, data):
        user = self.context['request'].user
        accountnumber = data.get("account").get('accountno') if data.get("account") else self.instance.account.accountno
        ...
        if nick_name_check and Internalbeneficiaries.objects.filter(account=account_obj, createdby=user,
...
class ExternalAPIListbeneficiarySerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        representation = super().to_representation(instance)
        representation['country_name'] = Countries.objects.get(shortform=instance.country.shortform).name
        representation['email']=instance.email if instance.email else ""
        return representation
    class Meta:
        model = Externalbeneficiaries
        fields = ("slug", "accountnumber", "name", "currency_code", "swiftcode", "city", "bankname", "country_code", "email")

class InternalAPIListbeneficiaryerializer(serializers.ModelSerializer):
    ...
    nickname=serializers.CharField(source="receivername")
    slug = ReadOnlyField()
    ...
    class Meta:
        model = Internalbeneficiaries
        fields = ("slug", "accountnumber", "first_name", "last_name", "currency_code",'nickname')


class InternalCreatebeneficySerializer(serializers.ModelSerializer):
    accountnumber = serializers.CharField(source="account.accountno")
    first_name = serializers.CharField(source="account.user_account.firstname")
    last_name = serializers.CharField(source="account.user_account.lastname")
    currency_code = ReadOnlyField(source="account.currency.code")
    ...
    def __init__(self, *args, **kwargs):
        super(InternalCreatebeneficySerializer, self).__init__(*args, **kwargs)
        if 'context' in kwargs:
            if 'is_company' in kwargs['context']:
                is_company = kwargs['context'].get('is_company')
                if is_company:
    ...
    def update(self, instance, validated_data):
        instance.account = Accounts.active.get(
            accountno=validated_data["account"].get('accountno', instance.account.accountno))
        instance.receivername = validated_data.get('receivername', instance.receivername)
        instance.save()
        return instance
    def validate_accountnumber(self, data):
    ...
    def validate(self, data):
        ...
        elif Internalbeneficiaries.objects.filter(createdby=user, receivername=receivername, isdeleted=False):
            raise serializers.ValidationError({"receivername": "Nick name already exist"})
