...
from Transactions.mixins import OTP, FindAccount, TransactionMail, add_log_action, checkNonAsciiChracters, transaction_lock_fn
import datetime
    ...
    def post(self, request):
        ...
        try:
            ...
            context = {
                ...
                'status':False,
                'accounts': Accounts.objects.filter(user_account__customer__user=request.user, isdeleted=False),
                'internalbeneficiaries' : Internalbeneficiaries.active.filter(createdby=request.user),
                'user_details' : Useraccounts.objects.get(isdeleted=False,customer__user=request.user)
            }
            # return redirect('accTransaction')
            return render(request,'accounttoaccount/ac-ac-transfer.html',context)
        if ben_acc_number in list(Accounts.objects.filter(accountno=ben_acc_number, isdeleted=False, createdby=request.user).values_list('accountno', flat=True)):
            context = {
                'data':prevdata,
                'messagee' : 'Debit and credit account cannot be from same user',
                'status':False,
                'accounts': Accounts.objects.filter(user_account__customer__user=request.user, isdeleted=False),
                'internalbeneficiaries' : Internalbeneficiaries.active.filter(createdby=request.user),
                'user_details' : Useraccounts.objects.get(isdeleted=False,customer__user=request.user)
            ...
            return render(request,'accounttoaccount/ac-ac-transfer.html',context)
        if not checkNonAsciiChracters(request.POST.get("note")):
            context = {
                'data':prevdata,
                'messagee' : 'Fancy Characters are not allowed',
                'status':False,
                'accounts': Accounts.objects.filter(user_account__customer__user=request.user, isdeleted=False),
                'internalbeneficiaries' : Internalbeneficiaries.active.filter(createdby=request.user),
                'user_details' : Useraccounts.objects.get(isdeleted=False,customer__user=request.user)
        ...
        if debit_account.user_account.test_account:
            if not Accounts.active.get(accountno=ben_acc_number).user_account.test_account:
                context = {
                    'data':prevdata,
                    'messagee' : 'Only transfer from test account to another test account is allowed',
                    'status':False,
                    'accounts': Accounts.objects.filter(user_account__customer__user=request.user, isdeleted=False),
                    'internalbeneficiaries' : Internalbeneficiaries.active.filter(createdby=request.user),
                    'user_details' : Useraccounts.objects.get(isdeleted=False,customer__user=request.user)
        ...
        if float(request.POST.get('amount')) <  float(debit_account.balance) :
            ...
            from_amount = round(Decimal(request.POST.get('amount')),2)
...
@method_decorator(login_required, name='dispatch')
@method_decorator(transaction_status, name='dispatch')
class AccountToAccountOTP(View,FindAccount):
            ...
            def validation_fn():
                ...
                if valid_date.date() <= otp.validtill:
                    ...
                    try:
                        ...
                        def create_transaction(transactionno,debit_account,credit_account,fromamount,toamount,debit_balance,credit_balance,amount_type,parent_transaction=None):
                            transaction_obj = Transactions.objects.create(
                                ...
                                conversionrate = request.session['accounttoaccounttransfer']['conversionrate'],
                                parenttransaction=parent_transaction
                            )
                            add_log_action(request, transaction_obj, status=f"transaction(Account to Account : amount type {transaction_obj.amount_type}) created for account {str(transaction_obj.fromaccount.accountno)}", status_id=1)
                            return transaction_obj
                        try:
                            net_amount = request.session.get("accounttoaccounttransfer").get("net_amount")
                            amount_type = "Net Amount"
                            parent_transaction = create_transaction(transactionno, debit_account, credit_account, net_amount,
                                               credit_amount,
                        ...
                        try:
                            master_credit_account,converted_conversion_fee = self.find_master_account_convert_amount(debit_account,conversion_fee,debit_account.user_account.test_account)
