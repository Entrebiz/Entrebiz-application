...
from utils.models import Accounts, Activitylog, Businessdetails, Useraccounts, Customerdocumentdetails, \
    Customerdocumentfiles, \
    Internationaltransactions, Cablecharges, InvoiceDocument, Transactions, Countries
import logging
...
def is_debit_or_credit(transaction, accountno):
    if transaction.transactiontype.name == "Third Party Transfer":
        ...
        if to_account and to_account == accountno:
            return 'debit'
    elif transaction.transactiontype.name == "International Transfer Fee to Master Account":
        if transaction.toaccount.user_account.ismaster_account:
            return 'credit'
    elif transaction.transactiontype.name == "Wallet Withdrawal Transfer Fee to Master Account":
        if transaction.toaccount.user_account.ismaster_account:
            return 'credit'
    else:
...
def get_reference(transaction):
    ...
    if transaction_type == "Third Party Transfer":
        ...
        if transaction.amount_type == "Wallet Withdrawal Fee":
            ...
            return f"Wallet Name - {get_recipient_accountnumber(transaction)}"
...
@register.simple_tag
def get_useraccounts(request):
    return Useraccounts.objects.get(isdeleted=False,customer__user=request.user)
...
@register.simple_tag
def get_affiliate_amount(transaction):
    try:
...
def get_balance_amount(transaction):
    ...
    if transaction_type == "International Transfer Fee":
        cable_charge = get_transaction_amounts(transaction).get("cable_charge")
        balance_amount -= cable_charge if cable_charge else 0
    if transaction_type == "Wallet Withdrawal Transfer Fee":
        cable_charge = get_transaction_amounts(transaction).get("cable_charge")
        balance_amount -= cable_charge if cable_charge else 0
    if balance_amount:
...
def enable_view_action(transaction,request):
    transaction_type = get_transaction_type(transaction)
    flag = False
    if request.user.customer_details.all():
        flag = request.user.customer_details.all()[0].useracc_customer.all()[0].ismaster_account
    if transaction_type in ["International Transfer", "Acccount To Account Transfer","Currency Conversion","Wallet Withdrawal Transfer"]:
        return True
    elif flag and transaction_type in ["Acccount To Account Transfer Fee","Currency Conversion Fee","International Transfer Fee","Wallet Withdrawal Transfer Fee"]:
        return True
    else:
        return False
...
def refund_show(int_transaction):
    ...
    if days_difference.days <= settings.FEE_CALCULATE_MINIMUM_DURATION and int_transaction.verificationstatus == "Executed":
        return True
    else:
        return False
...
@register.simple_tag
def refferal_accounts(accounts,isprimary,currency=None):
    if not currency:
        try:
            balance = accounts.get(isdeleted = False,isprimary=isprimary).balance
        except Exception as e:
            logger.info(e)
        return balance
    else:
        try:
            currency_code = accounts.get(isdeleted = False,isprimary=isprimary).currency.code
        except Exception as e:
            logger.info(e)
...
def country_name(code):
    try:
        country_name = Countries.objects.get(shortform=code).name
    except:
        country_name = ""
    return country_name