"""
    create test master user and accounts in USD and EUR currencies.
...
master_acc_email = "ebztestmaster@mailinator.com.com"
...
def create_master_user():
    ...
    try:
        user_account = Useraccounts.objects.get(ismaster_account=True,test_account=True)
    except:
        try:
            user = User.objects.get(email=master_acc_email)
        except:
            user = User.objects.create(email=master_acc_email, username=randomword(10),
                                       first_name="Entrebiz", last_name="Test Master")
        customer = Customers.objects.create(
            user=user, customertype=1, isactive=True)
        user_account = Useraccounts.objects.create(
            slug=randomword(10),
            customer=customer,
            ismaster_account=True,test_account=True, firstname="Entrebiz", middlename="Test Master",
...
def create_master_accounts():
    ...
    MASTER_ACC_NO = 20000102
    from django.db.models import Q
    currency_list = Currencies.objects.filter(Q(isdeleted=False, code="USD")|Q(isdeleted=False, code="EUR"))
    for currency in currency_list:
        try:
            ...
            account = Accounts.objects.create(slug=randomword(10),user_account=user_account,currency=currency,
                                              accounttype=2,balance=0,createdby=user_account.customer.user)
