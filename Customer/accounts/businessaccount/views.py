    ...
    def post(self,request):
        if request.session.get('businessaccount'):
            ...
            business_details = Businessdetails.objects.get_or_create(
                ...
                country_code=Countries.objects.filter(phonecode=request.session['businessaccount'].get('countryCode')).first(),
                address=request.session['businessaccount'].get('address'),
                company_country = Countries.objects.filter(shortform=request.session['businessaccount'].get('country')).first(),
                createdby=user,
            ...
            for user_each in users_lists:
                if users_lists.index(user_each) != indx:
                    ...
                    try:
                        if request.session.get('referFriend'):
                            refferer_account = Useraccounts.active.get(id=request.session['referFriend'].get('refferer_id'))
                            useraccount_each.referred_by = refferer_account
                            useraccount_each.save()
                    except Exception as e:
                        logger.info(e)
...
class CompanyDetailsView(View,OTP):
    def get(self,request):
        ...
        context={
            ...
            'country':business_details.company_country,
            'city':user_details.city,
            'state':user_details.region,
            'country_of_incorporation':business_details.company_country.name
            }
...
class EditCompanyDetailsView(View):
    def get(self,request):
        ...
        country_code = businessdetails.country_code.phonecode
        ...
        if businessdetails.country_code and businessdetails.country_code.isdeleted:
            context['country_disable_modal_show'] = True
    ...
    def post(self,request):
        try:
            ...
            Businessdetails.objects.filter(createdby=user).update(
                ...
                country_code=Countries.objects.filter(phonecode=request.session['companydetails']['countrycode']).first(),
                company_country=Countries.objects.filter(name=request.POST.get('countryOfIncorportaion')).first(),
    ...
    def post(self,request):
        if request.POST.get('action')=='add':
            ...
            if User.objects.filter(email=user_email).exists():
                ...
                current_user_reffered_by = Useraccounts.objects.get(customer__user=request.user).referred_by
                useraccount_each,status = Useraccounts.objects.get_or_create(customer=customer,firstname=request.POST.get('firstName'),
                        ...
                        referred_by=current_user_reffered_by,
                ...
                try:
                    ...
                    customer_files = Customerdocumentfiles.objects.create(
                                        customerdocument=customer_doc,filelocation=customer_doc_file_current.filelocation, document_type="Document"
                                    )
                    additional_document1 = Customerdocumentfiles.objects.filter(customerdocument=customer_doc_current, document_type="AdditionalDocument1").first()
                    additional_document2 = Customerdocumentfiles.objects.filter(customerdocument=customer_doc_current, document_type="AdditionalDocument2").first()
                    additional_document3 = Customerdocumentfiles.objects.filter(customerdocument=customer_doc_current, document_type="AdditionalDocument3").first()
                    if additional_document1:
                        Customerdocumentfiles.objects.create(
                                        customerdocument=customer_doc,filelocation=additional_document1.filelocation, document_type="AdditionalDocument1"
                                    )
                    if additional_document2:
                        Customerdocumentfiles.objects.create(
                                        customerdocument=customer_doc,filelocation=additional_document2.filelocation, document_type="AdditionalDocument2"
                                    )
                    if additional_document3:
                        Customerdocumentfiles.objects.create(
                                        customerdocument=customer_doc,filelocation=additional_document3.filelocation, document_type="AdditionalDocument3"
                                    )
                except:
                    pass
