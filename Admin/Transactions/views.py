...
from utils.models import (Currencies, Currencyconversionmargins,Externalbeneficiaries, Transactionpurposetype, Countries,
                          Accounts, Useraccounts, Otps, Transactiontypes, Businessdetails, Transactions,
                          Internationaltransactions, InvoiceDocument, Cablecharges, Customers, Currencyconversionratescombined)
from Transactions.mixins import OTP, TransactionMail, ModelQueries
...
class InternationalWireTransfer(View):
    ...
    def post(self,request):
        ...
        def conversion_charges(amount):
            ...
            elif amount >= 1000 and amount <7000 :
                self.total_tr_charge =Decimal(69.00)
                self.wire_transfer_fee = round(amount * Decimal((1/100)),4)
                self.cable_charge =round(self.total_tr_charge - self.wire_transfer_fee,4)
        ...
        def conversion_rates(amount, from_currency_code,conversion_fee, wire_transfer_fee, cable_charge):
            ...
            self.wire_transfer_fee = round(wire_transfer_fee * to_dollar.conversionrate,4)
            self.cable_charge = round(cable_charge * to_dollar.conversionrate,4)
            self.credit_amount = round(from_amount * conversionrate,4)
        ...
        commission_charges = round((self.wire_transfer_fee + self.cable_charge + self.conversion_fee),4)
        ...
        is_ext_ben = Externalbeneficiaries.objects.filter(accountnumber=request.POST.get('BeneficiaryACNo'),isdeleted=False).exists()
        request.session['internationalTransaction'].update({
                'cable_charge' : format(self.cable_charge, ".4f"),
                'wire_transfer_fee' : format(self.wire_transfer_fee, ".4f"),
                'conversion_fee' : format(self.conversion_fee, ".4f"),
                'debit_amount' : format(debit_amount, ".4f"),
                'commission_charges' : format(commission_charges, ".4f"),
                'credit_amount' : format(self.credit_amount, ".4f"),
...
class WireTransferConfirm(View):
    ...
    def post(self, request,token):
        if request.POST.get('action_type') == 'resent otp' :
            ...
            otp_status = OTP().send_email_otp(transaction_type=4,created_by=request.user,email=request.user.email,
    ...
    def post(self, request, token):
            ...
            def transaction_fn(last_transactionno, fromamount,charge,parent_tr=None,toamount=None, inl_tr=None, commission_charges=None,cable_charge=None,last_transaction=None):
                ...
                if cable_charge:
                    ...
                    if commission_charges:
                        ...
                        toaccount = None
                    # if commission_charges:
                    #     to_account_balance = to_account.balance+Decimal(commission_charges)
                    ...
                    try:
                        ...
                        if inl_tr:
                            try:
                                ...
                                logger.info(e)
                                pass
                            Internationaltransactions.objects.create(
                                                        ...
                                                        user_box_no=request.session['internationalTransaction']['Boxno'],
                                                        user_street=request.session['internationalTransaction']['Street'],
                                                        user_city=request.session['internationalTransaction']['userCity'],
                                                        user_state=request.session['internationalTransaction']['State'],
                                                        user_country=Countries.objects.get(name=request.session['internationalTransaction']['userCountry']),
                                                        user_phone=request.session['internationalTransaction']['Phoneno']
                                                        )
                    except Exception as e:
                        logger.info(e)
                        return {
        ...
        if response and not response.get('status'):
            ...
            context = {
                    ...
                    'email':request.session['internationalTransaction']['Email'],
                    'box_no':request.session['internationalTransaction']['Boxno'],
                    'street':request.session['internationalTransaction']['Street'],
                    'user_city':request.session['internationalTransaction']['userCity'],
                    'state':request.session['internationalTransaction']['State'],
                    'user_counrty':request.session['internationalTransaction']['userCountry'],
                    'phone_no':request.session['internationalTransaction']['Phoneno'],
...
class CurrencyConversionView(View,OTP):
    ...
    def post(self,request):
        ...
        net_amount = Decimal(float(request.POST.get("net_amount")))
        note = request.POST.get("note")
        conversion_fee = round(net_amount * Decimal(0.5 / 100),4)
        debit_amount = net_amount + conversion_fee
        if request.POST.get("debit_account") == request.POST.get("credit_account"):
            accounts = Accounts.objects.filter(user_account__customer__user=request.user, isdeleted=False)
            context['accounts'] = accounts
            context['balance'] = str(round(debit_account.balance, 4))
        ...
        credit_amount = round(net_amount * conversionrate,4)
        conversion_details = {
...
@method_decorator(login_required, name='dispatch')
class CurrencyConversionConfirmView(View,TransactionMail,OTP):
    def get(self,request):
    ...
    def post(self,request):
        ...
        if action_type == 'confirm-details':
            ...
            full_name = f"{user_account.firstname} {user_account.lastname}"
            context = request.session.get('currency_conversion')
            self.send_email_otp(transaction_type=2, created_by=request.user, email=request.user.email,
                                full_name=full_name,
                                token=request.session.get('currency_conversion').get('token'), activation_code=None)
            ...
            if not otp_obj.validated:
                if otp_obj.validtill >= datetime.datetime.now().date():
                    ...
                    try:
                        ...
                        try:
                            credit_account = Accounts.objects.get(user_account__ismaster_account=True,currency=debit_account.currency,isdeleted=False)
                            debit_account.balance = float(debit_account.balance) - float(conversion_fee)
                            credit_account.balance = float(credit_account.balance) + float(conversion_fee)
                            debit_account.save()
                            credit_account.save()
                            debit_balance = debit_account.balance
                            credit_balance = credit_account.balance
...
class ConversionSuccessView(View,TransactionMail):
    ...
    def post(self,request):
        ...
        context = {
            ...
            'conversionrate': format(float(request.session.get('currency_conversion').get('conversionrate')), ".4f"),
            'net_amount': format(float(request.session.get('currency_conversion').get('net_amount')), ".4f"),
            'note': request.session.get('currency_conversion').get('note'),
            'conversion_fee': format(float(request.session.get('currency_conversion').get('conversion_fee')), ".4f"),
            'debit_amount': format(float(request.session.get('currency_conversion').get('debit_amount')), ".4f"),
            'credit_amount': format(float(request.session.get('currency_conversion').get('credit_amount')), ".4f"),
...
class EStatementsView(View,ModelQueries):
    ...
    def post(self,request):
        ...
        if action_type == "gettransactiondetails":
            ...
            # response['account_balance'] = "{:,}".format(round(account.balance,4))
...
class GetTransactionDetailsView(View,ModelQueries,TransactionMail,UtilMixins):
    ...
    def post(self,request):
        ...
        if action_type == "get_trasnactiondetailsby_id":
            ...
            full_name = f'{transaction.fromaccount.user_account.firstname} {transaction.fromaccount.user_account.middlename} {transaction.fromaccount.user_account.lastname}'
            ...
            if not isExist:
                if transaction.transactiontype.name == "Third Party Transfer":
                    context = {
                        ...
                        'box_no': transaction.inltransaction_tr.all()[0].user_box_no,
                        'street': transaction.inltransaction_tr.all()[0].user_street,
                        'user_city': transaction.inltransaction_tr.all()[0].user_city,
                        'state': transaction.inltransaction_tr.all()[0].user_state,
                        'user_counrty': transaction.inltransaction_tr.all()[0].user_country.shortform,
                        'phone_no': transaction.inltransaction_tr.all()[0].user_phone,
                        ...
                        'credit_amount': transaction.toamount,
                        'note': transaction.note,
                        'year': str(transaction.createdon.year),
