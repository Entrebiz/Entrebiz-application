...
class Currency_ConversionSerializer(serializers.ModelSerializer):
    debit_account = serializers.CharField(required=True)
    credit_account = serializers.CharField(required=True)
    debit_acc_curr_code = serializers.CharField(source="debit_account.currency.code",required=True)
    net_amount = serializers.DecimalField(max_digits=18, decimal_places=4,required=True)
    ...
    def __init__(self, *args, **kwargs):
        super(Currency_ConversionSerializer, self).__init__(*args, **kwargs)
        if 'context' in kwargs:
            if 'currencyconversion' in kwargs['context']:
                is_type = kwargs['context'].get('currencyconversion')
                if is_type:
                    self.fields.pop('debit_acc_curr_code')
                    self.fields.pop('transaction_no')
    ...
    def validate(self,data,**kwargs):
        if not self.context.get('email_send'):
            try:
                debit_account_balance = Accounts.objects.get(accountno=int(data["debit_account"]))
            except Exception as e:
                message="Account does not exists"
                raise serializers.ValidationError({'debit_account':message})
            try:
                credit_account = Accounts.objects.get(accountno=data["credit_account"])
            except Exception as e:
                message="Account does not exists"
                raise serializers.ValidationError({'credit_account':message})
            debit_account_balance = Accounts.objects.get(accountno=int(data["debit_account"])).balance
    ...
    class Meta:
        model=Transactions
        fields = ('debit_account', 'credit_account', 'net_amount','note', 'debit_acc_curr_code','transaction_no')


class CheckoutSerializer(serializers.ModelSerializer):
    card_number = serializers.IntegerField(required = True)
    exp_month = serializers.IntegerField(required = True,error_messages = {"invalid": "A valid integer is required"})
    exp_year = serializers.IntegerField(required = True,error_messages = {"invalid": "A valid integer is required"})
    cvc = serializers.IntegerField(required = True,error_messages = {"invalid": "A valid integer is required"})
    amount = serializers.FloatField(required = True,error_messages = {"invalid": " A valid integer is required"})
    currency = serializers.CharField(required = True,error_messages = {"blank": "This field is required"})
    email=serializers.EmailField(required = True,error_messages = {"blank": "This field is required"})
    card_holder_name=serializers.CharField(required = True,error_messages = {"blank": "This field is required"})
    city = serializers.CharField(required = True,error_messages = {"blank": "This field is required"})
    state = serializers.CharField(required = True,error_messages = {"blank": "This field is required"})
    country = serializers.CharField(required = True,error_messages = {"blank": "This field is required"})
    postal_code = serializers.CharField(required = True,error_messages = {"invalid": "This field is required"})
    description = serializers.CharField(required = True,error_messages = {"blank": "This field is required"})
    line1 = serializers.CharField(required = True,error_messages = {"blank": "This field is required"})
    line2 =serializers.CharField(required = True,error_messages = {"blank": "This field is required"})
     
    class Meta:
        model = Stripe_Customer
        fields = ('card_holder_name','card_number','exp_month','exp_year','cvc','email','customer_id','token_id','amount','currency','city','state','postal_code','description','country','line1','line2')

    
class International_WireTransfer_Serializer(serializers.Serializer):
    debit_account = serializers.CharField(required=True)
    debit_acc_curr_code=serializers.CharField(required=True)
    beneficiary_account = serializers.CharField(required=True)
    ...
    bankswift_code = serializers.CharField(required=True)
    bank_city = serializers.CharField(required=True)
    bank_country = serializers.CharField(required=True)
    currency_code = serializers.CharField(required=True)
    user_box_no = serializers.CharField(required=True,error_messages = {"blank": "This field may not be blank."})
    user_street = serializers.CharField(required=True,error_messages = {"blank": "This field may not be blank."})
    user_city = serializers.CharField(required=True,error_messages = {"blank": "This field may not be blank."})
    user_state = serializers.CharField(required=True,error_messages = {"blank": "This field may not be blank."})
    user_country = serializers.CharField(required=True)
    user_phone = serializers.RegexField(required=True,regex=r'^\+?1?\d{9,15}$')
    purpose = serializers.CharField(required=True) 
    note = serializers.CharField(max_length=1000,required=False, allow_null=True,allow_blank=True)
    beneficiary_email = serializers.EmailField(max_length=100,required=False, allow_null=True,allow_blank=True)
    purpose_note = serializers.CharField(max_length=1000,required=False, allow_null=True,allow_blank=True)
    invoice_doc=serializers.FileField(required=False)
    has_invoice=serializers.CharField()
    ...
    def __init__(self, *args, **kwargs):
        super(International_WireTransfer_Serializer, self).__init__(*args, **kwargs)
        data = dict(kwargs['data'])
        if 'context' in kwargs:
            if 'InternationalWireTransfer' in kwargs['context']:
                is_type = kwargs['context'].get('InternationalWireTransfer')
                if is_type:
                    self.fields.pop('debit_acc_curr_code')
    ...
    def validate(self,data,**kwargs):
        if not self.context.get('email_send'):
            ...
            if data['debit_account']:
                account_exist= Accounts.objects.filter(accountno=data['debit_account']).exists()
                if not account_exist:
                    raise serializers.ValidationError({'debit_account':"Account does not exists"})
            if data['has_invoice']=='1':
                if 'invoice_doc' not in data:
                    message= "This field may not be blank."
                    raise serializers.ValidationError({'invoice_doc':message}) 
            if data['has_invoice']=='0':
                if 'invoice_doc'  in data:
                    message= "This field not required!."
                    raise serializers.ValidationError({'invoice_doc':message}) 
            if data['bank_country'] not in Countries.objects.values_list('name', flat=True):
                    message= "Invalid bank country name format!" 
                    raise serializers.ValidationError({'bank_country':message}) 
            if data['user_country'] not in Countries.objects.values_list('name',flat=True):
                    message= "Invalid user country name format!" 
                    raise serializers.ValidationError({'user_country':message}) 
        return data



class Personal_DetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model=Useraccounts
        fields=['slug','firstname','middlename','lastname','phonenumber','dateofbirth','street_address','city', 'region','country','nationality','zipcode']
