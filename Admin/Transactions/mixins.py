...
from utils.models import Currencyconversionratescombined, DomesticTransaction, Otps, Transactions, Useraccounts, Activationlinks, Accounts, Cablecharges, InvoiceDocument, \
    Internationaltransactions, Comments, WalletWithdrawalTransactions
import datetime
    ...
    def send_email_otp(self,transaction_type, created_by, email,full_name,token,activation_code=None):
        try:
            ...
            if transaction_type == 1:
                ...
                otp_type = "Currency Conversion"
                Purposevalue = "transaction initiated through"
            elif transaction_type == 3:
                otp_type = "Account to Account Transfer"
                Purposevalue = "transaction initiated through"
            elif transaction_type == 4:
                otp_type = "International Wire Transfer"
                Purposevalue = "transaction initiated through"
            elif transaction_type == 5:
                otp_type = "Login"
                Purposevalue = "login verification initiated through"
            elif transaction_type == 6:
                otp_type = "Change Password"
                Purposevalue = "password change initiated through"
            elif transaction_type == 7:
                otp_type = "Update User Details"
                Purposevalue = "user details updation initiated through"
            elif transaction_type == 8:
                otp_type = "Update Company Details"
                Purposevalue = "company details updation initiated through"
            else:
...
class TransactionMail:
    ...
    def mail_transfer_executed(self,recipient,context, wallet_tr=None):
        templatename = "wire-transfer-request/email/fund-transfer-approved-mail.html"
        if wallet_tr:
            templatename = "wallet-withdrawal-transfer-request/email/fund-transfer-approved-mail.html"
...
class ModelQueries:
    ...
    def get_transactions_filtered(self,transactions,accountid=None,transaction_no=None,beneficiary_name=None,from_date=None,to_date=None,creditdebit=None):
        ...
        if beneficiary_name:
            ...
            dom_transactions = transactions.filter(domtransaction_tr__accountholdername__icontains=beneficiary_name)
            ...
            transactions = transactions_to_be_appended | transactions | dom_transactions
        if from_date:
    ...
    def get_transaction_amounts(self,transaction,from_amount=False):
        ...
        try:
            ...
            cable_charge = None
        data = {
            'net_amount': transactions.filter(amount_type='Net Amount')[0].toamount if transactions.filter(
                amount_type='Net Amount') else None,
            'wire_fee': transactions.filter(amount_type='Wire Transfer Fee')[0].toamount if transactions.filter(
                amount_type='Wire Transfer Fee') else None,
            'conversion_fee': transactions.filter(amount_type='Conversion Fee')[0].toamount if transactions.filter(
            ...
            'domestic_fee': transactions.filter(amount_type='Domestic Transfer Fee')[0].toamount if transactions.filter(amount_type='Domestic Transfer Fee') else None,
            'cable_charge': cable_charge,
        }
        if from_amount:
            data['net_amount'] = transactions.filter(amount_type='Net Amount')[0].fromamount if transactions.filter(
                amount_type='Net Amount') else None
        return data
    ...
    def get_international_transaction_filtered(self,name=None,email=None,accountno=None,from_date=None,to_date=None,
                                               currencyid=None,tr_status=None, transfer_type="international_wire_transfer"):
        if transfer_type == "international_wire_transfer":
            int_transactions = Internationaltransactions.active.filter(hideforadmin=False)
        else:
            int_transactions = DomesticTransaction.active.filter(hideforadmin=False)
        if name:
            int_transactions = int_transactions.filter(transaction__createdby__customer_details__useracc_customer__fullname__icontains=name)
        if email:
        ...
        if accountno:
            ...
            int_transactions = int_transactions.filter(
                transaction__fromaccount__accountno=accountno)
        if from_date:
            from_date_obj = datetime.datetime.strptime(from_date, settings.DATE_FORMAT)
    ...
    def get_wallet_withdrawal_transaction_filtered(self,name=None,email=None,accountno=None,from_date=None,to_date=None,
                                               currencyid=None,tr_status=None):
        walltet_withdrawal_transactions = WalletWithdrawalTransactions.active.filter(hideforadmin=False)
        if name:
            walltet_withdrawal_transactions = walltet_withdrawal_transactions.filter(transaction__createdby__customer_details__useracc_customer__fullname__icontains=name)
        if email:
            walltet_withdrawal_transactions = walltet_withdrawal_transactions.filter(
                transaction__createdby__email=email)
        if accountno:
            walltet_withdrawal_transactions = walltet_withdrawal_transactions.filter(
        ...
        if from_date:
            from_date_obj = datetime.datetime.strptime(from_date, settings.DATE_FORMAT)
            walltet_withdrawal_transactions = walltet_withdrawal_transactions.filter(createdon__gte=from_date_obj.date())
        if to_date:
            to_date_obj = datetime.datetime.strptime(to_date, settings.DATE_FORMAT)
        ...
        if to_date_obj:
            to_date_obj = to_date_obj + datetime.timedelta(days=1)
            walltet_withdrawal_transactions = walltet_withdrawal_transactions.filter(createdon__lte=to_date_obj.date())

        if currencyid:
            walltet_withdrawal_transactions = walltet_withdrawal_transactions.filter(currency__id=currencyid)

        if tr_status:
            walltet_withdrawal_transactions = walltet_withdrawal_transactions.filter(verificationstatus=tr_status)

        return walltet_withdrawal_transactions
    ...
    def filter_user_account(self,name=None,email=None,accountno=None):
        ...
        if name:
            useraccounts = useraccounts.filter(fullname__icontains=name)
        if email:
        ...
        for userac in useraccounts:
            ...
            if userac.added_by:
                if userac.added_by.useracc_customer.all() and \
                    ...
                    accounts = userac.added_by.useracc_customer.all()[0].accnt_usr_accnt.filter(isdeleted=False)
            else:
                if userac.accnt_usr_accnt.all():
                    accounts = userac.accnt_usr_accnt.filter(isdeleted=False)
        ...
        if transaction_type in ["Inward Remittance", "Other Charges","Refund","Affiliate Fee","Other Charges Fee"]:
            ...
            try:
                if transaction_type == "International Transfer":
                    if transaction.amount_type == "Wire Transfer Fee":
                        last_transaction = Transactions.active.filter(transactionno=transaction.transactionno,amount_type="Wire Transfer Fee").order_by('-id')[
                    0]
                    elif transaction.amount_type == "Net Amount":
                        last_transaction = Transactions.active.filter(transactionno=transaction.transactionno,amount_type="Net Amount").order_by('-id')[
                    0]
                elif transaction_type == "Wallet Withdrawal Transfer":
                    if transaction.amount_type == "Wallet Withdrawal Fee":
                    ...
                    last_transaction = Transactions.active.filter(transactionno=transaction.transactionno).order_by('-id')[
                    0]
...
class TransactionMixins:
     def get_affiliate_amount(self,transaction):
        try:
            ...
            return None
        if wire_fee > 0:
            affiliate_amount = format(float(wire_fee) * 0.25, ".4f")
...
def add_log_action(request,model_obj,status,status_id):
    try:
...
class AccountActivationMail:
    def send_activation_mail(self,useraccount):
        if useraccount.added_by:
            customer = useraccount.added_by
            account_details =  Accounts.objects.filter(user_account__customer=customer, isdeleted=False).order_by('isprimary')
        else:
            account_details =  Accounts.objects.filter(user_account=useraccount, isdeleted=False).order_by('isprimary')
        account_list = []
        for account_detail in account_details:
            account_list.append(f"{account_detail.currency.code}")
        context = {
            'fullname': f"{useraccount.firstname} {useraccount.lastname}",
            'domain' : settings.CURRENT_DOMAIN,
            'primary_account': account_list[0],
            'secondary_account': account_list[1],
        }
        subject = 'Your Entrebiz account is active now'
        template = render_to_string('accounts/openaccount/personal-signup/accountactive.html',context)
        from_user = settings.EMAIL_HOST_USER
        to_user = [useraccount.customer.user.email,]
        def thread_send_custom_mail(subject,template,from_user,to_user):
            email = EmailMessage(subject,template,from_user,to_user)
            email.content_subtype = 'html'
            email.fail_silently=False
    ...
    def send_admin_approved_welcome_mail_touser(self,useraccount):
        if useraccount.added_by:
            customer = useraccount.added_by
            account_details =  Accounts.objects.filter(user_account__customer=customer, isdeleted=False).order_by('isprimary')
        else:
        ...
        account_list = []
        for account_detail in account_details:
            account_list.append(f"{account_detail.accountno} ({account_detail.currency.code})")
        fullname= f"{useraccount.firstname} {useraccount.lastname}"
        context = {
            'fullname': fullname,
            'domain' : settings.CURRENT_DOMAIN,
        ...
        subject = f"Welcome on board {fullname} - Entrebiz Pte. Ltd."
        template = render_to_string('accounts/openaccount/personal-signup/admin-approved-welcome-mail.html',context)
        from_user = settings.EMAIL_HOST_USER
        to_user = [useraccount.customer.user.email,]
        def thread_send_custom_mail(subject,template,from_user,to_user):
            email = EmailMessage(subject,template,from_user,to_user)
