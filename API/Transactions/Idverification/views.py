            ...
            if request.POST.get("check")=="Passport" or request.POST.get("check")=="Driving License":
                ...
                if not request.POST.get('docId'):
                    ...
                    try:
                        ...
                        if not ext1 in settings.ALLOWED_FORMATS or not ext2 in settings.ALLOWED_FORMATS or not ext3 in settings.ALLOWED_FORMATS:
                            return {
                                'status' : False,
                                'message' : 'Incorrect file format',
                                }
                        elif filesize1 > settings.MAX_FILE_SIZE or filesize2 > settings.MAX_FILE_SIZE or filesize3 > settings.MAX_FILE_SIZE:
                            return {
                                'status' : False,
                                'message' : 'Maximum file size allowed is 10 MB',
                                }
                        elif not all(ord(ch) < 128  for filename in [filename1,filename2,filename3] for ch in filename):
                            return {
                                'status' : False,
                                'message' : 'Special characters should not be in file name',
                                } 
