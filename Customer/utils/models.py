...
class Transactions(models.Model):
    ...
    ("Domestic Transfer Fee", "Domestic Transfer Fee"),
    ("Admin Fee", "Admin Fee"),
    )
...
#     useraccount = models.ForeignKey(Useraccounts, related_name='upipayment_useraccount', on_delete=models.CASCADE, blank=True, null=True)
#     qrcode = models.FileField(upload_to='upi_qrcode', blank=True, null=True)
#     upi_id = models.CharField(max_length=4000, blank=True, null=True)
...
class DomesticBeneficiary(models.Model):
    ACCOUNTTYPE = (
        ("Personal", "Personal"),
        ("Company", "Company"),
    )
    account_type = models.CharField(max_length=50, choices=ACCOUNTTYPE, default='Personal')
    slug = models.SlugField(unique=True)
    domestic_name = models.CharField(max_length=100)  # Field name made lowercase.
    domestic_accountnumber = models.CharField(max_length=50)  # Field name made lowercase.
    currency = models.ForeignKey(Currencies, related_name='dom_ben_crncy', on_delete=models.SET_NULL, blank=True,
                                 null=True)  # Field name made lowercase.
    routing_number = models.CharField(max_length=25)  # Field name made lowercase.
    country = models.ForeignKey(Countries,related_name='dom_ben_country',on_delete=models.SET_NULL, blank=True,null=True)  # Field name made lowercase.
    domestic_city = models.CharField(max_length=100)  # Field name made lowercase.
    domestic_bankname = models.CharField(max_length=200)  # Field name made lowercase.
    createdby = models.ForeignKey(User, related_name='dom_ben_created_by', on_delete=models.SET_NULL, blank=True,
                                  null=True)  # Field name made lowercase.
    createdon = models.DateTimeField(auto_now_add=True)  # Field name made lowercase.
    modifiedby = models.ForeignKey(User, related_name='dom_ben_modified_by', on_delete=models.SET_NULL, blank=True,
                                   null=True)  # Field name made lowercase.
    modifiedon = models.DateTimeField(auto_now=True)  # Field name made lowercase.
    isdeleted = models.BooleanField(default=False)  # Field name made lowercase.
    domestic_email = models.CharField(max_length=400, blank=True, null=True)  # Field name made lowercase.
    customer = models.ForeignKey(Customers, related_name='dom_ben_customer',
                                 on_delete=models.CASCADE)  # Field name made lowercase.
    objects = models.Manager()
    active = GetActiveManager()
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.slug:
            self.slug = slug
        super(DomesticBeneficiary, self).save(*args, **kwargs)
